...
```plaintext
$ kubectl exec --namespace=purelb lbnodeagent-ssblg -- microdnf install -y iproute
```

Now we can use the _ip_ command to examine the routes:
```plaintext
$ kubectl exec --namespace=purelb lbnodeagent-ssblg -- ip route show
...
```plaintext
$ kubectl exec --namespace=purelb lbnodeagent-ssblg -- ip addr show dev enp1s0
...
```plaintext
$ kubectl exec --namespace=purelb lbnodeagent-ssblg -- ip addr show dev kube-lb0
